    ...
    req.on('end',()=>{
        ...
        //error,results,fields:错误对象，json数组，数据信息数组
            ...
            for(let i=0;i<results.rows.length;i++){
                if(results.rows[i].account === data.account){
                    isregister = false;
                    break;
                }
            }
            if(isregister){
                ...
                pool.query('insert into users(account,nickname,password) values($1,$2,$3)',arr)
                .catch(err=>{
                    console.error(err)
                }); 
                res.send(db);
...
router.get('/delete',(req,res)=>{
    ...
    pool.query('DELETE FROM users WHERE account=$1',data)
    .catch(err=>{
        console.error(err)
    });
